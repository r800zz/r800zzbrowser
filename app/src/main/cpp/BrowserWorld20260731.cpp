...
struct BrowserWorld::State {
  ...
  bool chromaKeyEnabled = true; //r800zz
...
void //r800zz
BrowserWorld::ToggleChromaKey() { //r800zz
  ASSERT_ON_RENDER_THREAD(); //r800zz
  m.chromaKeyEnabled = !m.chromaKeyEnabled; //r800zz
} //r800zz

bool //r800zz
BrowserWorld::IsChromaKeyEnabled() const { //r800zz
  return m.chromaKeyEnabled; //r800zz
} //r800zz
...
BrowserWorld::DrawWorld(device::Eye aEye) {
  ...
  if (aEye == device::Eye::Left && m.layerBlitter) {
    for (const WidgetPtr& widget : m.widgets) {
      ...
      const bool chromaKeyEnabled = m.chromaKeyEnabled; //r800zz
      const bool flatToEquirectEnabled = isVRVideoWidget && m.flatToEquirectEnabled; //r800zz
      if (!widget->IsVisible() && !isVRVideoWidget) {
        continue;
      }
      widget->DrawLayerSurface(m.layerBlitter, chromaKeyEnabled, flatToEquirectEnabled); //r800zz
    }
  }
...
//m.device->SetImmersiveBlendMode( device::BlendMode::AlphaBlend);
  //m.device->SetImmersiveXRSessionType(m.externalVR->GetImmersiveXRSessionType());
//m.device->SetImmersiveXRSessionType( DeviceDelegate::ImmersiveXRSessionType::AR);

auto immersiveBlendMode = m.externalVR->GetImmersiveBlendMode();
auto immersiveXRSessionType = m.externalVR->GetImmersiveXRSessionType();
    m.device->SetImmersiveBlendMode(immersiveBlendMode);
...
JNI_METHOD(void, toggleChromaKeyNative) //r800zz
(JNIEnv*, jobject) { //r800zz
  crow::BrowserWorld::Instance().ToggleChromaKey(); //r800zz
} //r800zz
