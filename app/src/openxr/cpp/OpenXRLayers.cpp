...
// OpenXRLayerEquirect;
    ...
    OpenXRLayerEquirect::Init(JNIEnv * aEnv, XrSession session, vrb::RenderContextPtr& aContext) {
        ...
        for (auto& xrLayer: xrLayers)
            xrLayer = { XR_TYPE_COMPOSITION_LAYER_EQUIRECT2_KHR };

        OpenXRLayerBase<VRLayerEquirectPtr, XrCompositionLayerEquirect2KHR>::Init(aEnv, session, aContext);
    }

    void
    OpenXRLayerEquirect::Destroy() {
        swapchain = nullptr;
        OpenXRLayerBase<VRLayerEquirectPtr, XrCompositionLayerEquirect2KHR>::Destroy();
    }
    ...
    OpenXRLayerEquirect::Update(XrSpace aSpace, const XrPosef &aReorientPose, XrSwapchain aClearSwapChain) {
        ...
        OpenXRLayerBase<VRLayerEquirectPtr, XrCompositionLayerEquirect2KHR>::Update(aSpace, aReorientPose, aClearSwapChain);
        ...
        for (int i = 0; i < numXRLayers; ++i) {
            ...
            xrLayers[i].radius = 0;
//
xrLayers[i].centralHorizontalAngle = layer->GetCentralHorizontalAngle();
xrLayers[i].upperVerticalAngle = layer->GetUpperVerticalAngle();
xrLayers[i].lowerVerticalAngle = layer->GetLowerVerticalAngle();
//
