...
import static com.r800zz.r800zzbrowser.ui.widgets.UIWidget.REMOVE_WIDGET;
...
import com.r800zz.r800zzbrowser.audio.AndroidMediaPlayer;
import com.r800zz.r800zzbrowser.audio.AndroidMediaPlayerSoundPool;
import com.r800zz.r800zzbrowser.audio.AudioEngine;
import com.r800zz.r800zzbrowser.browser.Accounts;
import com.r800zz.r800zzbrowser.browser.Media;
import com.r800zz.r800zzbrowser.browser.PermissionDelegate;
import com.r800zz.r800zzbrowser.browser.SettingsStore;
import com.r800zz.r800zzbrowser.browser.api.WRuntime;
import com.r800zz.r800zzbrowser.browser.api.WSession;
import com.r800zz.r800zzbrowser.browser.engine.EngineProvider;
import com.r800zz.r800zzbrowser.browser.engine.Session;
import com.r800zz.r800zzbrowser.browser.engine.SessionStore;
import com.r800zz.r800zzbrowser.geolocation.GeolocationWrapper;
import com.r800zz.r800zzbrowser.input.MotionEventGenerator;
...
import com.r800zz.r800zzbrowser.telemetry.OpenTelemetry;
import com.r800zz.r800zzbrowser.telemetry.TelemetryService;
import com.r800zz.r800zzbrowser.ui.OffscreenDisplay;
import com.r800zz.r800zzbrowser.ui.adapters.Language;
import com.r800zz.r800zzbrowser.ui.widgets.AbstractTabsBar;
import com.r800zz.r800zzbrowser.ui.widgets.AppServicesProvider;
import com.r800zz.r800zzbrowser.ui.widgets.HorizontalTabsBar;
import com.r800zz.r800zzbrowser.ui.widgets.KeyboardWidget;
import com.r800zz.r800zzbrowser.ui.widgets.NavigationBarWidget;
import com.r800zz.r800zzbrowser.ui.widgets.NotificationManager;
import com.r800zz.r800zzbrowser.ui.widgets.OverlayContentWidget;
import com.r800zz.r800zzbrowser.ui.widgets.RootWidget;
import com.r800zz.r800zzbrowser.ui.widgets.TrayWidget;
import com.r800zz.r800zzbrowser.ui.widgets.UISurfaceTextureRenderer;
import com.r800zz.r800zzbrowser.ui.widgets.UIWidget;
import com.r800zz.r800zzbrowser.ui.widgets.VerticalTabsBar;
import com.r800zz.r800zzbrowser.ui.widgets.WebXRInterstitialWidget;
import com.r800zz.r800zzbrowser.ui.widgets.Widget;
...
import com.r800zz.r800zzbrowser.ui.widgets.WindowWidget;
import com.r800zz.r800zzbrowser.ui.widgets.Windows;
import com.r800zz.r800zzbrowser.ui.widgets.dialogs.LegalDocumentDialogWidget;
import com.r800zz.r800zzbrowser.ui.widgets.dialogs.PromptDialogWidget;
import com.r800zz.r800zzbrowser.ui.widgets.dialogs.SendTabDialogWidget;
import com.r800zz.r800zzbrowser.ui.widgets.dialogs.WhatsNewWidget;
import com.r800zz.r800zzbrowser.ui.widgets.menus.VideoProjectionMenuWidget;
import com.r800zz.r800zzbrowser.utils.BitmapCache;
import com.r800zz.r800zzbrowser.utils.ConnectivityReceiver;
import com.r800zz.r800zzbrowser.utils.DeviceType;
...
import com.r800zz.r800zzbrowser.browser.api.WResult;
...
import android.database.Cursor; //r800zz
import android.provider.OpenableColumns; //r800zz

/*
import com.google.mlkit.vision.segmentation.Segmenter; //r800zz
import com.google.mlkit.vision.segmentation.Segmentation; //r800zz
import com.google.mlkit.vision.segmentation.selfie.SelfieSegmenterOptions; //r800zz
import com.google.mlkit.vision.common.InputImage; //r800zz
*/
import android.graphics.Bitmap; //r800zz
import java.io.ByteArrayOutputStream; //r800zz MODNet
import ai.onnxruntime.OrtEnvironment; //r800zz MODNet
import ai.onnxruntime.OrtSession; //r800zz MODNet
import ai.onnxruntime.OnnxTensor; //r800zz MODNet
import ai.onnxruntime.OrtLoggingLevel; //r800zz MODNet QNN debug
import ai.onnxruntime.OrtEpDevice; //r800zz
import java.nio.ByteBuffer; //r800zz MODNet
import java.nio.ByteOrder; //r800zz MODNet
import java.nio.FloatBuffer; //r800zz MODNet
import java.util.Collections; //r800zz MODNet
import java.util.concurrent.ExecutorService; //r800zz MODNet
import java.util.concurrent.Executors; //r800zz MODNet
import java.util.HashMap; //r800zz MODNet QNN
import android.system.Os; //r800zz MODNet QNN HTP
import java.io.IOException; //r800zz MODNet QNN HTP
...
private static final int CHROMA_KEY_OFF = 0; //r800zz
private static final int CHROMA_KEY_GREEN = 1; //r800zz
private static final int CHROMA_KEY_AUTO = 2; //r800zz
private static final int CHROMA_KEY_BLACK = 3; //r800zz
private static final int CHROMA_KEY_WHITE = 4; //r800zz
private static final int CHROMA_KEY_SKY_BLUE = 5; //r800zz
private static final int AI_PASSTHROUGH = 6; //r800zz

private int mChromaKeyMode = CHROMA_KEY_GREEN; //r800zz
//private Segmenter mSelfieSegmenter; //r800zz goole.milkit
private boolean mSegmentationBusy = false; //r800zz
/* temporal mask retention
private byte[] mPreviousSegmentationMask; //r800zz
private int mPreviousSegmentationMaskWidth = 0; //r800zz
private int mPreviousSegmentationMaskHeight = 0; //r800zz
*/
private OrtEnvironment mOrtEnvironment; //r800zz MODNet
private OrtSession mModnetSession; //r800zz MODNet
private final ExecutorService mModnetExecutor = Executors.newSingleThreadExecutor(); //r800zz MODNet
private volatile boolean mModnetBusy = false; //r800zz MODNet

    public static final String CUSTOM_URI_SCHEME = "r800zzbrowser";
    public static final String CUSTOM_URI_HOST = "com.r800zz.r800zzbrowser";
    ...
    private boolean mIsChromaKeyEnabled = true; //r800zz
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //SelfieSegmenterOptions options = new SelfieSegmenterOptions.Builder() .setDetectorMode(SelfieSegmenterOptions.STREAM_MODE) .build(); //r800zz

        //mSelfieSegmenter = Segmentation.getClient(options); //r800zz
        //Log.e("R800ZZ_AI", "Selfie Segmenter created: " + mSelfieSegmenter); //r800zz
        initializeModnet(); //r800zz MODNet
    }
...
//
        final int initialChromaKeyMode = mChromaKeyMode; //r800zz
        final int initialChromaKeyWindowHandle =
          mWindows.getFocusedWindow().getHandle(); //r800zz
         queueRunnable(() -> setChromaKeyModeNative(
          initialChromaKeyMode,
          initialChromaKeyWindowHandle)); //r800zz
//
    ...
    private void initializeSpeechRecognizer() {
        try {
            ...
            Log.e(LOGTAG, "Exception creating the speech recognizer: " + e); ((VRBrowserApplication) getApplication()).setSpeechRecognizer(null);
        }
    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        //  see https://github.com.r800zz.r800zzbrowser/issues/797
    ...
    public void checkTogglePassthrough() {
        if (mSettings.isStartWithPassthroughEnabled() && !mIsPassthroughEnabled) {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(this::togglePassthrough, 1000); //r800zz
        }
    }
    ...
    @Override //r800zz
    public void toggleGreenChromaKey() { //r800zz
        if (mChromaKeyMode == CHROMA_KEY_GREEN) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = CHROMA_KEY_GREEN; //r800zz
        } //r800zz
    
        final int mode = mChromaKeyMode; //r800zz
        final int windowHandle = mWindows.getFocusedWindow().getHandle(); //r800zz
        queueRunnable(() -> setChromaKeyModeNative(mode, windowHandle)); //r800zz
    } //r800zz

    @Override //r800zz
    public void toggleBlackChromaKey() { //r800zz
        if (mChromaKeyMode == CHROMA_KEY_BLACK) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = CHROMA_KEY_BLACK; //r800zz
        } //r800zz
    ...
    @Override //r800zz
    public void toggleWhiteChromaKey() { //r800zz
        if (mChromaKeyMode == CHROMA_KEY_WHITE) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = CHROMA_KEY_WHITE; //r800zz
        } //r800zz
    ...
    @Override //r800zz
    public void toggleSkyBlueChromaKey() { //r800zz
        if (mChromaKeyMode == CHROMA_KEY_SKY_BLUE) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = CHROMA_KEY_SKY_BLUE; //r800zz
        } //r800zz
    ...
    @Override //r800zz
    public void toggleAiPassthrough() { //r800zz
        if (mChromaKeyMode == AI_PASSTHROUGH) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = AI_PASSTHROUGH; //r800zz
        } //r800zz
    ...
    @Override //r800zz
    public void toggleAutoColorChromaKey() { //r800zz
        if (mChromaKeyMode == CHROMA_KEY_AUTO) { //r800zz
            mChromaKeyMode = CHROMA_KEY_OFF; //r800zz
        } else { //r800zz
            mChromaKeyMode = CHROMA_KEY_AUTO; //r800zz
        } //r800zz
    ...
    @Override //r800zz
    public boolean isChromaKeyEnabled() { //r800zz
        return mChromaKeyMode != CHROMA_KEY_OFF; //r800zz
    } //r800zz

    @Override //r800zz
    public int getChromaKeyMode() { //r800zz
      return mChromaKeyMode; //r800zz
    } //r800zz
    ...
    private native void setAiSegmentationMaskNative(byte[] mask, int width, int height); //r800zz
    private native void setChromaKeyModeNative(int mode, int windowHandle); //r800zz
    ...
    private void showErrorMessage(String message) {
        runOnUiThread(() -> {
            ...
            Log.i(LOGTAG, "!!DIAGNOSTIC!!: " + message);
        });
    }

    private Uri copyToTempFile(Uri sourceUri, File tempDir) {
        try {
            // Get the original file name
            String fileName = "upload_" + System.currentTimeMillis();
            try (android.database.Cursor cursor = getContentResolver().query(sourceUri, null, null, null, null)) {
            ...
            Log.d(LOGTAG, "File copied to temp: " + destFile.getAbsolutePath());
            return Uri.fromFile(destFile);
        } catch (Exception e) {
            Log.e(LOGTAG, "Failed to copy file to temp", e);
            return null;
        }
    }
    ...
    private WSession.PromptDelegate.FilePrompt mCurrentFilePrompt;
    private WResult<WSession.PromptDelegate.PromptResponse> mCurrentFileResult;
    private static final int FILE_CHOOSER_REQUEST_CODE = 9999;
    private static final int VIDEO_FILE_CHOOSER_REQUEST_CODE = 10000; //r800zz
    private static final int FILE_MANAGER_REQUEST_CODE = 10001; //r800zz

    public void openSystemFileChooser(WSession.PromptDelegate.FilePrompt prompt, WResult<WSession.PromptDelegate.PromptResponse> result) {
        mCurrentFilePrompt = prompt;
        mCurrentFileResult = result;

        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*");
        if (prompt.type() == WSession.PromptDelegate.FilePrompt.Type.MULTIPLE) {
        ...
        try {
            ...
            Log.e(LOGTAG, "Failed to open system file chooser", e);
            result.complete(prompt.dismiss());
        }
    }

    public void openFileManager() { //r800zz
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT); //r800zz
        intent.addCategory(Intent.CATEGORY_OPENABLE); //r800zz
        intent.setType("*/*"); //r800zz
    
        try { //r800zz
            startActivityForResult( //r800zz
                    Intent.createChooser(intent, "File Manager"), //r800zz
                    FILE_MANAGER_REQUEST_CODE); //r800zz
        } catch (Exception e) { //r800zz
            Log.e(LOGTAG, "Failed to open file manager", e); //r800zz
            showErrorMessage("Failed to open file manager"); //r800zz
        } //r800zz
    } //r800zz

    private void openSelectedVideo(Uri sourceUri) { //r800zz
        File cacheDir = getCacheDir(); //r800zz
    
        // Remove previously copied temporary videos. //r800zz
        File[] oldFiles = cacheDir.listFiles(); //r800zz
        if (oldFiles != null) { //r800zz
            for (File oldDir : oldFiles) { //r800zz
                if (oldDir.isDirectory() &&
                        (oldDir.getName().equals("temp_video") ||
                         oldDir.getName().startsWith("temp_video_"))) { //r800zz
                    File[] children = oldDir.listFiles(); //r800zz
                    if (children != null) { //r800zz
                        for (File child : children) child.delete(); //r800zz
                    } //r800zz
                    oldDir.delete(); //r800zz
                } //r800zz
            } //r800zz
        } //r800zz
    
        File tempDir = new File(cacheDir, "temp_video_" + System.currentTimeMillis()); //r800zz
        if (!tempDir.exists()) tempDir.mkdirs(); //r800zz
    
        Uri videoUri = copyToTempFile(sourceUri, tempDir); //r800zz
        Session session = SessionStore.get().getActiveSession(); //r800zz
    
        if (videoUri != null && session != null) { //r800zz
            session.loadUri(videoUri.toString()); //r800zz
        } else { //r800zz
            showErrorMessage("Failed to load video file"); //r800zz
        } //r800zz
    } //r800zz

    private String getSelectedFileName(Uri uri) { //r800zz
        Cursor cursor = null; //r800zz
    
        try { //r800zz
            cursor = getContentResolver().query( //r800zz
                    uri, //r800zz
                    new String[]{OpenableColumns.DISPLAY_NAME}, //r800zz
                    null, //r800zz
                    null, //r800zz
                    null); //r800zz
    
            if (cursor != null && cursor.moveToFirst()) { //r800zz
                int index = cursor.getColumnIndex(OpenableColumns.DISPLAY_NAME); //r800zz
                if (index >= 0) { //r800zz
                    return cursor.getString(index); //r800zz
                } //r800zz
            } //r800zz
        } catch (Exception e) { //r800zz
            Log.e(LOGTAG, "Failed to get selected file name", e); //r800zz
        } finally { //r800zz
            if (cursor != null) cursor.close(); //r800zz
        } //r800zz
    
        return null; //r800zz
    } //r800zz

    private void prepareQnnHtp() throws Exception { //r800zz MODNet QNN HTP
        File qnnDir = new File(getFilesDir(), "qnn"); //r800zz
        if (!qnnDir.exists() && !qnnDir.mkdirs()) { //r800zz
            throw new IOException("Failed to create QNN HTP directory"); //r800zz
        } //r800zz
    
        File skelFile = new File(qnnDir, "libQnnHtpV73Skel.so"); //r800zz
    
        try (InputStream in = getAssets().open("qnn/libQnnHtpV73Skel.so"); //r800zz
             OutputStream out = new FileOutputStream(skelFile)) { //r800zz
            byte[] buffer = new byte[8192]; //r800zz
            int length; //r800zz
            while ((length = in.read(buffer)) != -1) { //r800zz
                out.write(buffer, 0, length); //r800zz
            } //r800zz
        } //r800zz
    
        String adspPath = qnnDir.getAbsolutePath(); //r800zz
        String oldAdspPath = Os.getenv("ADSP_LIBRARY_PATH"); //r800zz
        if (oldAdspPath != null && !oldAdspPath.isEmpty()) { //r800zz
            adspPath += ";" + oldAdspPath; //r800zz
        } //r800zz
        Os.setenv("ADSP_LIBRARY_PATH", adspPath, true); //r800zz
    } //r800zz

    private void initializeModnet() { //r800zz MODNet
        try {
            mOrtEnvironment = OrtEnvironment.getEnvironment();
Log.e("R800ZZ_MODNET", "ORT version: " + mOrtEnvironment.getVersion()); //r800zz MODNet
Log.e("R800ZZ_MODNET", "Available providers: " + OrtEnvironment.getAvailableProviders()); //r800zz MODNet QNN
    
            ByteArrayOutputStream output = new ByteArrayOutputStream();
    
            try (InputStream input = getAssets().open("modnet/modnet_webcam_256x256.onnx")) {
            ...
            byte[] modelBytes = output.toByteArray();
    
            OrtSession.SessionOptions sessionOptions = new OrtSession.SessionOptions();

//sessionOptions.setSessionLogLevel( OrtLoggingLevel.ORT_LOGGING_LEVEL_VERBOSE); //r800zz MODNet QNN debug
//sessionOptions.setSessionLogVerbosityLevel(4); //r800zz MODNet QNN debug

            Map<String, String> qnnOptions = new HashMap<>(); //r800zz MODNet QNN
              qnnOptions.put("backend_path", "libQnnGpu.so"); //r800zz MODNet QNN
//qnnOptions.put("log_severity_level", "0"); //r800zz MODNet QNN GPU
              sessionOptions.addQnn(qnnOptions); //r800zz MODNet QNN
              sessionOptions.addConfigEntry("session.disable_cpu_ep_fallback", "1"); //r800zz MODNet QNN debug

            mModnetSession =
                    mOrtEnvironment.createSession(modelBytes, sessionOptions);
//
for (Map.Entry<String, ai.onnxruntime.NodeInfo> entry :
        mModnetSession.getInputInfo().entrySet()) { //r800zz MODNet
    Log.e("R800ZZ_MODNET",
            "Input: " + entry.getKey() + " " + entry.getValue()); //r800zz
}

for (Map.Entry<String, ai.onnxruntime.NodeInfo> entry :
        mModnetSession.getOutputInfo().entrySet()) { //r800zz MODNet
    Log.e("R800ZZ_MODNET",
            "Output: " + entry.getKey() + " " + entry.getValue()); //r800zz
}
//
    
            Log.e("R800ZZ_MODNET", "MODNet session created"); //r800zz
        } catch (Exception e) {
            Log.e("R800ZZ_MODNET", "MODNet initialization failed", e); //r800zz
        }
    }

    //no need "@Override"
    public void requestR800zzExit() { //r800zz
      showAppExitDialog(); //r800zz
    } //r800zz

    public void openVideoFileChooser() { //r800zz
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT); //r800zz
        intent.addCategory(Intent.CATEGORY_OPENABLE); //r800zz
        intent.setType("video/*"); //r800zz

        try { //r800zz
            startActivityForResult( //r800zz
                    Intent.createChooser(intent, "Select Video File"), //r800zz
                    VIDEO_FILE_CHOOSER_REQUEST_CODE); //r800zz
        } catch (Exception e) { //r800zz
            Log.e(LOGTAG, "Failed to open video file chooser", e); //r800zz
            showErrorMessage("Failed to open video file chooser"); //r800zz
        } //r800zz
    } //r800zz

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        Log.i(LOGTAG, "!!DIAGNOSTIC!! onActivityResult reached. Request: " + requestCode + ", Result: " + resultCode);
        super.onActivityResult(requestCode, resultCode, data);
        if (requestCode == VIDEO_FILE_CHOOSER_REQUEST_CODE) { //r800zz
            if (resultCode == RESULT_OK && data != null && data.getData() != null) { //r800zz
                openSelectedVideo(data.getData()); //r800zz
            } //r800zz
            return; //r800zz
        } //r800zz
        if (requestCode == FILE_MANAGER_REQUEST_CODE) { //r800zz
            if (resultCode == RESULT_OK && data != null && data.getData() != null) { //r800zz
                Uri uri = data.getData(); //r800zz
                String mimeType = getContentResolver().getType(uri); //r800zz
                String fileName = getSelectedFileName(uri); //r800zz
        
                boolean isVideo = mimeType != null && mimeType.startsWith("video/"); //r800zz
                boolean isApk = fileName != null && fileName.toLowerCase().endsWith(".apk"); //r800zz
        
                if (isVideo) { //r800zz
                    openSelectedVideo(uri); //r800zz
                } else { //r800zz
                    Intent openIntent; //r800zz
        
                    if (isApk) { //r800zz
                        openIntent = new Intent(Intent.ACTION_INSTALL_PACKAGE); //r800zz
                        openIntent.setDataAndType( //r800zz
                                uri, //r800zz
                                "application/vnd.android.package-archive"); //r800zz
                    } else { //r800zz
                        openIntent = new Intent(Intent.ACTION_VIEW); //r800zz
                        openIntent.setDataAndType( //r800zz
                                uri, //r800zz
                                mimeType != null ? mimeType : "*/*"); //r800zz
                    } //r800zz
        
                    openIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); //r800zz
        
                    try { //r800zz
                        startActivity(openIntent); //r800zz
                    } catch (Exception e) { //r800zz
                        Log.e(LOGTAG, "No application can open the selected file", e); //r800zz
                        showErrorMessage("No application can open this file"); //r800zz
                    } //r800zz
                } //r800zz
            } //r800zz
            return; //r800zz
        ...
        if (requestCode == FILE_CHOOSER_REQUEST_CODE) {
            if (mCurrentFileResult == null || mCurrentFilePrompt == null) {
                showErrorMessage("Result error: mCurrentFileResult or Prompt is null");
                return;
            }

            if (resultCode == RESULT_OK && data != null) {
                ArrayList<Uri> uris = new ArrayList<>();
                File tempDir = new File(getCacheDir(), "temp_upload");
                if (!tempDir.exists()) tempDir.mkdirs();

                if (data.getData() != null) {
                    Uri copiedUri = copyToTempFile(data.getData(), tempDir);
                    if (copiedUri != null) {
                        uris.add(copiedUri);
                    } else {
                        showErrorMessage("Error: Failed to copy file");
                    }
                } else if (data.getClipData() != null) {
                    for (int i = 0; i < data.getClipData().getItemCount(); i++) {
                        Uri copiedUri = copyToTempFile(data.getClipData().getItemAt(i).getUri(), tempDir);
                        if (copiedUri != null) uris.add(copiedUri);
                    }
                }

                if (!uris.isEmpty()) {
                    showErrorMessage("Success! Prepared: " + uris.size() + " file(s)");
                    mCurrentFileResult.complete(mCurrentFilePrompt.confirm(this, uris.toArray(new Uri[0])));
                } else {
                    showErrorMessage("Error: No files processed");
                    mCurrentFileResult.complete(mCurrentFilePrompt.dismiss());
                }
            } else {
                showErrorMessage("Selection cancelled (ResultCode: " + resultCode + ")");
                mCurrentFileResult.complete(mCurrentFilePrompt.dismiss());
            }
            mCurrentFileResult = null;
            mCurrentFilePrompt = null;
        }
    }

    public void processModnetFrame(byte[] pixels, int width, int height) { //r800zz MODNet
        if (mModnetSession == null || mOrtEnvironment == null || mModnetBusy) {
            return;
        }
//Log.e("R800ZZ_MODNET", "Providers during inference: " + OrtEnvironment.getAvailableProviders()); //r800zz MODNet QNN
    
        if (width <= 0 || height <= 0 ||
                width % 32 != 0 || height % 32 != 0) {
            Log.e("R800ZZ_MODNET",
                    "Invalid MODNet input size: " + width + "x" + height);
            return;
        }
    
        mModnetBusy = true;
    
        mModnetExecutor.execute(() -> {
            try {
                int planeSize = width * height;
    
                ByteBuffer byteBuffer =
                        ByteBuffer.allocateDirect(
                                planeSize * 3 * Float.BYTES)
                                .order(ByteOrder.nativeOrder());
    
                FloatBuffer inputBuffer = byteBuffer.asFloatBuffer();
    
                for (int i = 0, p = 0; i < planeSize; i++, p += 4) {
                    float r = ((pixels[p] & 0xFF) / 127.5f) - 1.0f;
                    float g = ((pixels[p + 1] & 0xFF) / 127.5f) - 1.0f;
                    float b = ((pixels[p + 2] & 0xFF) / 127.5f) - 1.0f;
    
                    inputBuffer.put(i, r);
                    inputBuffer.put(planeSize + i, g);
                    inputBuffer.put(planeSize * 2 + i, b);
                }
...
//long modnetStart = System.currentTimeMillis(); //r800zz MODNet
                     ...
                     OrtSession.Result result =
                             mModnetSession.run(
                                     Collections.singletonMap(
                                             "input",
                                             inputTensor))) {
//Log.e("R800ZZ_MODNET", "Inference time: " + (System.currentTimeMillis() - modnetStart) + " ms"); //r800zz MODNet
    
                    OnnxTensor outputTensor =
                            (OnnxTensor) result.get(0);
    
                    FloatBuffer matte =
                            outputTensor.getFloatBuffer();
    
                    if (matte == null || matte.remaining() < planeSize) {
                        throw new IllegalStateException(
                                "Invalid MODNet output");
                    }
    
                    byte[] byteMask = new byte[planeSize];
    
                    for (int i = 0; i < planeSize; i++) {
                        float value = matte.get();
    
                        byteMask[i] =
                                (byte) (value >= 0.5f ? 255 : 0); //r800zz MODNet
                    }
    
                    mHandler.post(() -> {
                        setAiSegmentationMaskNative(
                                byteMask,
                                width,
                                height); //r800zz MODNet
                ...
                Log.e("R800ZZ_MODNET",
                        "MODNet inference failed", e); //r800zz MODNet
                mModnetBusy = false;
            }
        });
    }
